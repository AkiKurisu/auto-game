...
{
    /// <summary>
    /// Small helper surface for Unity Player snippets executed through AutoGame.
    /// </summary>
    public static class Probe
    {
        ...
        public static UComponent[] Components(Type componentType, bool includeInactive = true)
        {
            ...
            return FindComponentObjects(componentType, includeInactive)
                ...
                .ThenBy(c => Path(c.transform), StringComparer.Ordinal)
        ...
        /// </summary>
        public static ProbeMember[] Members(
            string typeName,
            string filter = null,
            ...
            int limit = 80)
        {
            ...
            foreach (var field in EnumerateFields(type, flags))
            {
                if (!NameMatches(field.Name, loweredFilter))
                    continue;
                members.Add(new ProbeMember("field", field.Name, FormatType(field.FieldType), field.IsStatic));
            }
    ...
    /// <summary>
    /// Describes a reflected member returned by <see cref="Probe.Members(object, string, bool, int)"/>.
    /// </summary>
