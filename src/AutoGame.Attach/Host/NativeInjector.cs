...
internal static class NativeInjector
{
        ...
        try
        {
            ...
            try
            {
                ...
                if (offset < 0 || offset >= localModule.Size)
                    throw new PlayerTargetException("UnityAttachBootstrapExportInvalid", "Bootstrap export is outside the loaded module.");
                ...
                if (remote.Size != localModule.Size || !SameFile(remote.Path, nativePath))
                    throw new PlayerTargetException("UnityAttachBootstrapIdentityMismatch", "Loaded bootstrap file identity or size differs from the requested DLL.");
                var code = CallBootstrap(handle, remote.Base + offset, payloadPath + "\n" + config, attempt);
                attempt.Step("bootstrap-result", new { code });
                if (code != 0) throw new PlayerTargetException("UnityAttachBootstrapFailed", $"Native Bootstrap returned stage {code}.");
            }
        ...
        try
        {
            ...
            try
            {
                ...
                if (exit != 0 || completed != 1)
                    throw new PlayerTargetException("UnityAttachLoaderIncomplete", $"Remote loader did not complete normally (thread exit 0x{exit:X8}, marker {completed}). Do not replay.");
                if (module == 0)
                {
                    attempt.NoRemoteEffect();
                    throw new PlayerTargetException("UnityAttachLoadLibraryFailed", $"Remote LoadLibraryW failed: Win32 {error} ({new Win32Exception((int)error).Message}).");
                }
                return module;
            }
            finally
    ...
    internal static Module Unique(IEnumerable<Module> candidates, string role, AttachAttempt attempt)
    {
        ...
        if (found.Length != 1)
            throw new PlayerTargetException("UnityAttach" + role + (found.Length == 0 ? "NotFound" : "Ambiguous"),
    ...
    internal static nint ResolveExport(nint address, Module[] local, Module[] target, AttachAttempt attempt)
    {
        ...
        if (owner.Size != remote.Size || offset < 0 || offset >= remote.Size || !SameFile(owner.Path, remote.Path))
            throw new PlayerTargetException("UnityAttachSystemModuleMismatch", $"Cannot safely map {owner.Name} export to target module.");
        ...
        string stage, uint timeoutMs, ref bool canFree)
    {
        ...
        try
        {
            ...
            if (wait != 0)
            {
                ...
                throw new PlayerTargetException("UnityAttachRemoteTimeout", $"{stage}: remote thread did not finish in {timeoutMs} ms. Outcome unknown; do not replay.");
            }
            canFree = true;
    ...
    private static PlayerTargetException Win32(string api, AttachAttempt attempt)
    {
        var error = Marshal.GetLastWin32Error();
        attempt.Log("win32-failure", new { api, win32Error = error });
        return new PlayerTargetException("UnityAttach" + api + "Failed", $"{attempt.Stage}: {api} failed: Win32 {error} ({new Win32Exception(error).Message}).");
    }
