...
internal sealed class AttachAttempt(int pid, string? journal = null, string? diagnosticsPath = null)
{
    ...
    private readonly string? diagnostics = diagnosticsPath ?? Environment.GetEnvironmentVariable("AUTOGAME_ATTACH_DIAGNOSTICS");
    ...
    internal void Failed(Exception error)
    {
        Log("failure", new { errorType = error.GetType().FullName,
            errorCode = (error as PlayerTargetException)?.Code,
    ...
    internal PlayerTargetException DescribeFailure(Exception error)
    {
        var code = (error as PlayerTargetException)?.Code ?? (Stage switch
        {
        ...
        return new PlayerTargetException(code, $"{Stage}: {error.Message}");
    }
