...
internal static class TargetCompiler
{
    public static string Compile(string sourcePath, IEnumerable<string> references, string cache, string prefix = "Attach_", string generation = "")
    {
        ...
        var identity = "CSharp9|AUTOGAME_ATTACH|Debug|" + generation + source + string.Join("\n", paths.Select(p => p + Convert.ToHexString(SHA256.HashData(File.ReadAllBytes(p)))));
        ...
        var compilation = CSharpCompilation.Create(prefix + hash,
            Directory.Exists(sourcePath)
                ? Directory.GetFiles(sourcePath, "*.cs").Order().Select(p => CSharpSyntaxTree.ParseText(File.ReadAllText(p), new CSharpParseOptions(LanguageVersion.CSharp9, preprocessorSymbols: new[] { "AUTOGAME_ATTACH" }), p))
                : new[] { CSharpSyntaxTree.ParseText(source, new CSharpParseOptions(LanguageVersion.CSharp9, preprocessorSymbols: new[] { "AUTOGAME_ATTACH" }), sourcePath) },
