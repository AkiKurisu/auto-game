...
internal static class AttachStorage
{
    private static string NormalizeFinalPath(string path)
    {
        ...
        if (path.StartsWith(@"\\?\", StringComparison.Ordinal) || path.StartsWith(@"\\.\", StringComparison.Ordinal))
            throw new PlayerTargetException("UnityAttachPathResolutionFailed", "The resolved device path cannot be passed to Unity Mono.");
        return path;
    }
    ...
    internal static string ResolveFile(string path, string purpose, AttachAttempt? attempt = null)
    {
        ...
        catch (Exception error) when (error is IOException or UnauthorizedAccessException or Win32Exception or ArgumentException or NotSupportedException)
        {
            ...
            throw new PlayerTargetException("UnityAttachPathResolutionFailed", $"Cannot resolve {purpose}: {path}. {error.Message}");
        }
    }
    ...
    internal static void ValidateMonoPath(string path, string purpose, int suffixLength = 0)
    {
        if (path.Length + suffixLength >= 260)
            throw new PlayerTargetException("UnityAttachPathTooLong", $"{purpose} actual path requires {path.Length + suffixLength} characters; Unity Mono requires fewer than 260: {path}");
    }
    ...
    internal static void CheckStateAliases(string logical, string physical, AttachAttempt attempt)
    {
        foreach (var suffix in new[] { "", ".bootstrap", ".lifecycle", ".error" })
        {
            ...
            if ((a && !b) || (a && b && !NativeInjector.SameFile(first, second)))
                throw new PlayerTargetException("UnityAttachPathStateConflict", $"Logical and physical Attach state differ: {first}; {second}. No bootstrap was sent.");
        }
    }
    ...
    internal static string ProductVersion => typeof(PlayerAttachService).Assembly.GetName().Version!.ToString(3);
    ...
    internal static string RuntimeIdentity(string nativePath)
    {
        ...
        var assembly = typeof(PlayerAttachService).Assembly;
    ...
    internal static string DefaultRoot => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AutoGame", "attach");

    internal static string ExtractNative(string root)
    {
        using var stream = typeof(PlayerAttachService).Assembly.GetManifestResourceStream("AutoGame.Native.dll")
        ...
        var path = Path.Combine(directory, "AutoGame.Native.dll");
        if (!File.Exists(path))
        {
            var temporary = path + "." + Guid.NewGuid().ToString("N");
    ...
    internal static string Connection(string root, int pid)
    {
        ...
        return Path.Combine(root, "sessions", $"{pid}-{process.StartTime.ToUniversalTime().Ticks}.bridge");
    }
    ...
    internal static string? DataPath(string path)
    {
        try
        {
            var value = WireProtocol.Decode(File.ReadAllBytes(path)).AsObject();
            return value.TryGetValue("dataPath", out var dataPath) ? dataPath.AsString() : null;
        }
        catch (Exception e) when (e is IOException or InvalidDataException or ArgumentException) { return null; }
    }
}
